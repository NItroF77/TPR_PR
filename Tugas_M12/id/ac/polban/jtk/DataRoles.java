...
class EngineerTools {
    ...
    void removeDataOpsTools() {
    	try {
    		this.EngTools.removeAll(this.DataOpsTools);//remove all DataOpsTools within EngTools
    		List<String> temp = new ArrayList<String>(this.EngTools); //create a clone of EngTools
    		temp.retainAll(DataOpsTools); //remove all that's not within DataOpsTools at temp
    		assert temp.isEmpty():"Failed to remove!"; //is temp empty?
    	}
        catch (Exception e) {
